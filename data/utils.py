...
def split_ref_data(data_root,shot, record_file='./data/refsplits.json'):
    '''
    根据 shot 数量动态划分参考图像。
    划分规则：每个类别随机打乱25张图，取前 `25-shot` 张为训练集，剩余 `shot` 张为测试集。
    '''
    ...
    for cate in cates:
        ...
        random.shuffle(ref_cate_image_names)# 随机打乱
        ...
        # 修正：前 25-shot 张训练，后 shot 张测试
        train_size = 25 - shot
        ref_cate_train_samples = [name.split('.')[0] for name in ref_cate_image_names[:train_size]]
        ref_cate_test_samples = [name.split('.')[0] for name in ref_cate_image_names[train_size:]]

        refsplits['train'][cate] = ref_cate_train_samples
        refsplits['test'][cate] = ref_cate_test_samples

    with open(record_file, 'w') as f:
        json.dump(refsplits, f, indent=4) #indent=4：第三个参数，可选，用于设置 JSON 文件的缩进为 4 个空格
    print(f'>>> Generated {record_file} with shot={shot}')


# def collect_r2c_data(data_root, mode='train', record_file='./data/refsplits.json'):
def collect_r2c_data(data_root, shot, mode='train', record_file_prefix='./data/refsplits'):
    '''
    根据 shot 动态收集数据。
    为不同 shot 生成不同的划分文件，避免实验间干扰。
    '''
    # 为不同 shot 实验生成不同的记录文件
    record_file = f'{record_file_prefix}_shot{shot}.json'

    # if not os.path.exists(record_file):
    #     split_ref_data(data_root, record_file)
    if not os.path.exists(record_file):
        split_ref_data(data_root, shot, record_file)  # 传入 shot 参数
    ...
    assert len(camo_classes) == len(ref_classes) == 64 ##可注释
