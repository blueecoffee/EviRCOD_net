    ...
    with torch.no_grad():
        with tqdm(total=len(test_loader)) as pbar:
            for (image, gt, sal_f, name) in test_loader:
                ...
                sal_f = sal_f.cuda()
                _, _, _, _,res = model(x=image, ref_x=sal_f, y=None, training=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='saved_models')
    ...
    params_path = os.path.join(opt.save_root, opt.model_name, 'Net_epoch_bestscore.pth')
