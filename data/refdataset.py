...
# 在数据加载时，需要加载原始参考图像而不是预提取的特征
class RefCODDataset(Dataset):
    def __init__(self, camo_root, ref_root, split_file, transform=None):
        self.camo_root = camo_root
    ...
    def __getitem__(self, idx):
        ...
        ref_names = self.splits['train'][category]  # 根据训练/测试选择
        selected_refs = random.sample(ref_names, 5)  # 随机选择5张
        
        ref_imgs = []
        for ref_name in selected_refs:
            ref_path = os.path.join(self.ref_root, category, f"{ref_name}.jpg")
            ref_img = Image.open(ref_path).convert('RGB')
            if self.transform:
                ref_img = self.transform(ref_img)
            ref_imgs.append(ref_img)
        
        ref_imgs = torch.stack(ref_imgs)  # (5, 3, H, W)
        
        return camo_img, ref_imgs, gt_mask
...
class R2CObjData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.shot > 0 or self.shot == -1:           
            ...
            num_aux = len(file_class_chosen)# 训练时：25-shot，验证时：shot
            ref_feat_list = []

            if self.mode == 'train':
                # 训练时从训练集（25-shot张）中随机选择shot张
            ...
            for idx in ref_idx_list:
                ...
                # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n")
                # # 打印npy文件路径和对应的形状                
                # print(f"Loading npy file: {file_class_chosen[idx]}, shape: {ref_feat.shape}")
